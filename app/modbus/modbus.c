#include "modbus.h"

static unsigned char table_hig[] =
{
	
};

static unsigned char table_low[] =
{
	
};

unsigned short crc_modbus(unsigned char *data, int len)
{
	unsigned int	i = 0;
	unsigned char hig = 0xFF;
	unsigned char low = 0xFF;

	while (len--)
	{
		i = low ^ (*data++);
		low = hig ^ table_hig[i];
		hig = table_low[i];
	}
	return ((unsigned short)(hig << 8) | (unsigned short)(low & 0xff));
}

struct data{
	int a;
	int b;
};

int modbus_data_check(rtu_request_t *pRequestMsg, int len)
{
	if(pRequestMsg == NULL)
	{
		return -1;
	}

	if(len != 5)
	{
		return Error_Format;
	}

	//check slaveaddr
	if(pRequestMsg->request.data[SLAVE_INDEX] != SLAVE_INDEX)
	{
		return Error_SlaveAddr;
	}

	//check func
	if(pRequestMsg->request.data[FUNCTION_INDEX] != x11_read_tim || pRequestMsg->request.data[FUNCTION_INDEX] != x12_write_tim)
	{
		return Error_Func;
	}

	//check crc
	if(MakeShort(pRequestMsg->request.data[3], pRequestMsg->request.data[4]) != crc_modbus(pRequestMsg->request.data, 3))
	{
		return Error_Crc;
	}

	return 0;
}


